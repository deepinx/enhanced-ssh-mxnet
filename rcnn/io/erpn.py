...
from eimage import get_image, tensor_vstack, get_crop_image
...
def get_crop_batch(roidb):
    ...
    for i in range(len(imgs)):
      ...
      if roidb[i]['gt_classes'].size > 0:
          ...
          gt_landmarks = np.empty((roidb[i]['landmarks'].shape[0], 10), dtype=np.float32)
          gt_landmarks[:, 0:10] = roidb[i]['landmarks'][gt_inds, :]
      else:
          gt_boxes = np.empty((0, 5), dtype=np.float32)
          gt_landmarks = np.empty((0, 10), dtype=np.float32)
...
def assign_anchor_fpn(feat_shape, gt_boxes, gt_landmarks, im_info):
    """
    assign ground truth boxes to anchor positions
    :param feat_shape: infer output shape
    :param gt_boxes: assign ground truth
    ...
    if gt_boxes.size > 0:
        bbox_targets[:] = bbox_transform(anchors, gt_boxes[argmax_overlaps, :4])
        kpoint_targets[:] = landmark_transform(anchors, gt_landmarks[argmax_overlaps, :10])
