...
from rcnn.io.erpn import get_rpn_testbatch, get_rpn_batch, assign_anchor, assign_anchor_fpn, get_crop_batch
...
class EAnchorLoader(mx.io.DataIter):
    def __init__(self, feat_sym, roidb, batch_size=1, shuffle=False, ctx=None, work_load_list=None,
        ...
        :return: AnchorLoader
        """
        super(EAnchorLoader, self).__init__()
...
class EAnchorLoaderFPN(mx.io.DataIter):
    def __init__(self, feat_sym, roidb, batch_size=1, shuffle=False, ctx=None, work_load_list=None,
                 aspect_grouping=False):
        """
        This Iter will provide roi data to Fast R-CNN network
        ...
        :return: AnchorLoader
        """
        super(EAnchorLoaderFPN, self).__init__()
    ...
    def infer_shape(self, max_data_shape=None, max_label_shape=None):
        ...
        label_dict = assign_anchor_fpn(feat_shape_list, dummy_boxes, dummy_landmarks, dummy_info)
        label_list = []
    ...
    def get_batch(self):
        ...
        for data, label in zip(data_list, label_list):
            ...
            for im_i in range(config.TRAIN.BATCH_IMAGES):
                ...
                label_dict = assign_anchor_fpn(feat_shape_list, gt_boxes, gt_landmarks, im_info, vis)
...
class ECropLoader(mx.io.DataIter):
    def __init__(self, feat_sym, roidb, batch_size=1, shuffle=False, ctx=None, work_load_list=None,
                 aspect_grouping=False):
        """
        This Iter will provide roi data to Fast R-CNN network
        ...
        :return: AnchorLoader
        """
        super(ECropLoader, self).__init__()
        ...
        if config.TRAIN.RPN_ENABLE_OHEM<0:
          self.label_name = ['label', 'bbox_target', 'bbox_weight', 'kpoint_target']
        else:
          #self.label_name = ['label', 'bbox_target', 'bbox_weight']
          self.label_name = []
          names = ['label', 'bbox_target', 'bbox_weight', 'kpoint_target']
          #names = ['label', 'bbox_weight']
          for stride in self.feat_stride:
            for n in names:
              k = "%s_stride%d"%(n,stride)
    ...
    def get_batch(self):
        ...
        for data, label in zip(data_list, label_list):
            ...
            #print('im_info', im_info.shape)
            #print(gt_boxes.shape)
            label_dict = assign_anchor_fpn(feat_shape_list, gt_boxes, gt_landmarks, im_info)
            for k in self.label_name:
